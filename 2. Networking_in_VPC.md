...
- [5. Practice: Creating subnets and set up NACL and Security Group](#5-practice-creating-subnets-and-set-up-nacl-and-security-group)
...
- **Private subnets**: do not have a route to the Internet Gateway and cannot directly accessed from the internet.
...
# 5. Practice: creating subnets and set up NACL and Security Group 
...
Now we have three subnets, but they are all still private since we haven't attach route tables to them yet.
...
2. With public subnet: After creation → **Edit routes** → add Route to our internet gateway we created earlier. With private subnets, do not need to do anything more since it by default has route to internal VPC network.
...
1. Search for **Security Groups** → Create a new security group → Add inbound and outbound rules as needed. In this case, I allow communication on port **22** (SSH), **80** (HTTP), **443** (HTTPS) from any IP address.
...
3. Create a key pair, download it (`.pem` file) and store it in a safe place. This key pair is used to connect to the EC2 instance via SSH. (This is asymmetric encryption, AWS hold the public key, and the key you have just downloaded is the private key)
...
1. Search for **RDS**. vWe will leanr more about RDS in the next lession, for now, just simply understand that RDS is a managed database service by AWS.

2. Before, we create databse, we must first create database subnet group, which let AWS know which subnet to put the database server in. Click on **Subnet groups** → Create **DB subnet group** → Name it, choose the VPC → Choose the private subnet that we created earilier.
...
4. Create the database: Click on **Databases** → Choose the database engine you want, in this case, I choose **MySQL** → Choose the **free tier** → Choose our VPC, DB subnet group, and security group we created earlier → Enter the database name, username, password
...
**Finnaly, remember to stop or terminal the EC2 and RDS instances to avoid unnecessary charges.**
